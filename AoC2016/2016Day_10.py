data = """bot 59 gives low to bot 176 and high to bot 120
...
def solve(data):
    data = sorted(data.splitlines(), key=lambda s:len(s.split()))
    bots = defaultdict(list)
    for m in data:

        if len(m.split()) <= 6:
            m = m.split()
            print(bots, int(m[-1]), int(m[1]))
            bots[int(m[-1])].append(int(m[1]))
    seen = set()
    outs = defaultdict(list)
    while not all([len(vals)==2 for vals in bots.values()]):
        to_check = deepcopy(list(bots.items()))
        for b, val in to_check:
            if b not in seen:
                if len(val)==2:
                    for m in data:
                        m = m.split()
                        if int(m[1])==b and len(m) > 6:
                            if "output" in m:
                                ind = m.index("output")+1
                                outs[m[ind]] = min(val) if ind==6 else max(val)

                            bots[int(m[6])].append(min(val))
                            bots[int(m[6])] = list(set(bots[int(m[6])]))
                            bots[int(m[-1])].append(max(val))
                            bots[int(m[-1])] = list(set(bots[int(m[-1])]))
                            seen.add(b)
        print(outs)
        print(bots[103], bots[35])
        """       
        ...
        #2x31
        #Low from bot 50 (low from 103 (High from bot 0, , high from 35 (high 142)
