        ...
        for inst in lines[line]:
            if inst[0]=="D" or inst[0]=="U":
                for new in range(int(inst[1:])):
                    pos[1] += d[inst[0]][1]
                    G.add_edge((pos[0], pos[1]-d[inst[0]][1], line), (pos[0], pos[1], line))

            if inst[0]=="L" or inst[0]=="R":
                for new in range(int(inst[1:])):
                    pos[0] += d[inst[0]][0]
                    G.add_edge((pos[0]-d[inst[0]][0], pos[1], line), (pos[0], pos[1], line))
    inter = []
    for node in G.nodes:
        if (node[0], node[1], 1-node[-1]) in G.nodes and node != (0,0,0) and node!=(0,0,1):
            inter.append(node[:-1])
    m = 9999
    m2 = 999999
    for each in inter:
        a = distance(*each, 0,0)
        r1 = nx.shortest_path_length(G, (0,0,0), (*each,0))
        r2 = nx.shortest_path_length(G, (0,0,1), (*each,1))
        if r1+r2 < m2:
            print(m2)
            m2 = r1+r2
        if a < m:
