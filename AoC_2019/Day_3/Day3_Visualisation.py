        ...
        for line in range(len(lines)):
            ...
            for inst in lines[line]:
                ...
                if inst[0]=="D" or inst[0]=="U":
                    for new in range(int(inst[1:])//yscale_down):
                        pos[1] += d[inst[0]][1]
                        positions[tuple(pos)].add(line)
                        screen.set_at(tuple(pos), colours[line])
                        c += 1
                        #screen.set_at((pos[0]+1, pos[1]), colours[line])
                if inst[0]=="L" or inst[0]=="R":
                    for new in range(int(inst[1:])//xscale_down):
                        pos[0] += d[inst[0]][0]
                        positions[tuple(pos)].add(line)
                        screen.set_at(tuple(pos), colours[line])
                        c += 1
                        #screen.set_at((pos[0], pos[1]+1), colours[line])
                pygame.display.flip()
            clock.tick(400)
